...
async def test_download_model_success(temp_dir):
    ...
    with patch(
        "model_filemanager.create_model_path",
        return_value=("models/checkpoints/model.sft", "model.sft"),
    ), patch("model_filemanager.check_file_exists", return_value=None), patch(
        "folder_paths.folder_names_and_paths", fake_paths
    ...
    )  # At least start, one progress update, and completion

    # Check initial call
    mock_progress_callback.assert_any_call(
        "model.sft",
        DownloadModelStatus(
            DownloadStatusType.PENDING, 0, "Starting download of model.sft", False
        ),
    )
    ...
    mock_progress_callback.assert_any_call(
        ...
        DownloadModelStatus(
            ...
            100,
            "Successfully downloaded model.sft",
            False,
        ),
    )
...
async def test_download_model_url_request_failure(temp_dir):
    ...
    with patch(
        "model_filemanager.create_model_path",
        return_value="/mock/path/model.safetensors",
    ), patch("model_filemanager.check_file_exists", return_value=None), patch(
        "folder_paths.folder_names_and_paths", fake_paths
    ):
        # Call the function
...
async def test_check_file_exists_when_file_exists(tmp_path):
    ...
    result = await check_file_exists(
        str(file_path), "existing_model.sft", mock_callback
    )
    ...
    mock_callback.assert_called_once_with(
        ...
        DownloadModelStatus(
            DownloadStatusType.COMPLETED,
            100,
            "existing_model.sft already exists",
...
async def test_check_file_exists_when_file_does_not_exist(tmp_path):
    ...
    result = await check_file_exists(
        str(file_path), "non_existing_model.sft", mock_callback
    )
...
async def test_track_download_progress_no_content_length(temp_dir):
    ...
    result = await track_download_progress(
        mock_response, full_path, "model.sft", mock_callback, interval=0.1
    )
...
async def test_track_download_progress_interval(temp_dir):
    ...
    ]  # This should be enough for 10 chunks

    full_path = os.path.join(temp_dir, "model.sft")

    with patch("time.time", mock_time):
        await track_download_progress(
            mock_response, full_path, "model.sft", mock_callback, interval=1.0
        )
    ...
    ), f"Expected at least 3 calls, but got {mock_callback.call_count}"
    ...
    assert (
        0 <= first_call[0][1].progress_percentage < 50
    ), f"First call progress was {first_call[0][1].progress_percentage}%"
