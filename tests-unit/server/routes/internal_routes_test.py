...
async def test_list_files_invalid_directory(aiohttp_client_factory, internal_routes):
    internal_routes.file_service.list_files = MagicMock(
        side_effect=ValueError("Invalid directory key")
    )
    client = await aiohttp_client_factory()
...
def test_setup_routes(internal_routes):
    ...
    assert any(
        route.method == "GET" and str(route.path) == "/files" for route in routes
    )
...
async def test_file_service_initialization():
    with patch(
        "api_server.routes.internal.internal_routes.FileService"
    ) as MockFileService:
